...
class OdriveCalibrationNode : public rclcpp::Node
{
public:
    OdriveCalibrationNode() : Node("odrive_calib")
    {
        RCLCPP_INFO(this->get_logger(), "=== ODrive Calibration Node ===");
        ...
        // Tạo các motor với ID 0,1,2,3,4,5
        std::vector<uint8_t> device_id_calib = {0, 1, 2, 3, 4, 5};
        

        for (auto id : device_id_calib)
        {
...
private:
    ...
    void executeCalib()
    {
        RCLCPP_INFO(this->get_logger(), "Starting Full Calibration sequence...");
        ...
        // Bước 2: Gửi lệnh fullcalib
        RCLCPP_INFO(this->get_logger(), "Step 3: Sending calib command...");
        for (auto &motor : motors_)
        {
            if (motor->fullCalibration())
            {
                RCLCPP_INFO(this->get_logger(), "  Device %d: calibration started", motor->getDeviceId());
            }
            else
            {
                RCLCPP_ERROR(this->get_logger(), "  Device %d: Failed to start calib", motor->getDeviceId());
            }
            std::this_thread::sleep_for(std::chrono::milliseconds(50));
        }


        // Bước 3: Chờ homing hoàn thành (5 giây)
        ...
        // // Bước 4: Bật closed loop control
...
int main(int argc, char **argv)
{
    ...
    auto node = std::make_shared<OdriveCalibrationNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}